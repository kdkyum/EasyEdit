...
def get_unembedding_matrix(
  ...
  if key not in _tensor_cache:
    ...
    elif isinstance(model, (transformers.LlamaForCausalLM, transformers.Qwen2ForCausalLM, transformers.MistralForCausalLM)):
...
def get_final_ln(model: transformers.PreTrainedModel) -> torch.nn.Module:
  ...
  if isinstance(
      model, (transformers.LlamaForCausalLM, transformers.Qwen2ForCausalLM, transformers.MistralForCausalLM)
  ):
...
def get_layer_names(model: transformers.PreTrainedModel) -> list[str]:
  ...
  if isinstance(
      model, (transformers.LlamaForCausalLM, transformers.Qwen2ForCausalLM, transformers.MistralForCausalLM, transformers.GptOssForCausalLM, transformers.Olmo2ForCausalLM)
  ):
...
def get_messages_start_role(model_name_or_path: str) -> str:
  ...
  # OpenAI gpt-oss models use the harmony/chat template; start with user
